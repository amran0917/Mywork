...
void splitNonLeaf(Block *curBlock){
   ...
   for(int k=0;k<(rightBlock->tNodes+1);k++) rightBlock->value[k]=rightBlock->value[k+1];
...
void redistributeBlock(Block *leftBlock, Block *rightBlock, bool isLeaf, int posOfLeftBlock, int whichOneisCurBlock){
    ...
    string PrevRightFirstVal = rightBlock->value[0];
    ...
    if(whichOneisCurBlock==0){ //leftBlock is curBlock
        ...
        if(!isLeaf){
            ...
            memcpy(&rightBlock->value[0], &rightBlock->value[1], sizeof(string)*(rightBlock->tNodes+1));
        ...
        if(!isLeaf){
            ...
            memcpy(&rightBlock->value[1], &rightBlock->value[0], sizeof(string)*(rightBlock->tNodes+1));
            ...
            leftBlock->value[leftBlock->tNodes-1] = c;
...
void mergeBlock(Block *leftBlock, Block *rightBlock, bool isLeaf, int posOfRightBlock){
    ...
    memcpy(&leftBlock->value[leftBlock->tNodes], &rightBlock->value[0], sizeof(string)*(rightBlock->tNodes+1));
    ...
    memcpy(&leftBlock->parentBlock->value[posOfRightBlock-1], &leftBlock->parentBlock->value[posOfRightBlock], sizeof(string)*(leftBlock->parentBlock->tNodes+1));
...
bool dataFound;
void deleteNode(Block *curBlock, string val, int curBlockPosition){
    ...
    string prevLeftMostVal = curBlock->value[0];
     ...
     for(int i=0;dataFound==false &&  i<=curBlock->tNodes; i++){
        ...
        //if we could find the target value at any leafBlock then
            ...
            memcpy(&curBlock->value[i], &curBlock->value[i+1], sizeof(string)*(curBlock->tNodes+1));
            //decrease number of nodes by one
            curBlock->tNodes--;
            dataFound = true;
            break;
...
int searchPos(string pvalue,string *root_arr,int n)
{
	int pos=0;
	while (pos < n && pvalue > root_arr[pos])
        pos++;
	return pos;
...
/*void searchKey(string pvalue)
{
	...
	while(1){
	  int n	= root->tNodeশানালামs;
		for(int i=0;i<root->tNodes;i++)
		{
			//cout<<root->value[i]<<endl;		
	//	}  
                
               if(root->value[i] == pvalue)
            {
              //  keyfound = true;
               // return;
               cout<<"found"<<endl;
            } 
            if(pvalue < root->value[i] && root->childBlock[i]!=NULL)
            {
                searchKey(root->childBlock[i], pvalue);
               
                cout<<"found"<<endl;
            }
      } 
	/*	pos = searchPos(pvalue,root->value,n);
		
		if (pos < n && pvalue == root->value[pos])
        {
            printf("Key %s found in position %d of last dispalyed node\n",pvalue,i);
            return;
        }
         root->value[pos]; 
    }
		cout<<pvalue<<" is not available"<<endl;
...
void print(vector < Block* > Blocks){
    ...
    for(int i=0; i<Blocks.size(); i++){ //for every block
       ...
       cout <<"[|";
        ...
        if(curBlock->value[j]==c && curBlock->childBlock[j]!=NULL)
            newBlocks.push_back(curBlock->childBlock[j]);
...
int main()
{
	...
	if(input.is_open()){
     ...
     cout<<"NUmber value " << w_count << endl;   
    ...
    clock_t q,r;
	  ...
	  while(cin>>pvalue)
       {
       q =clock();
    	searchKey(rootBlock,pvalue);
    	r=clock();
    	cout<<"Reqire time "<< ((r-q)/double(CLOCKS_PER_SEC))*1000<<" ms"<<endl;
    	if(keyfound)
    	{
